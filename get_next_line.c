...
/*   Updated: 2024/02/26 10:20:30 by asayad           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buff[(size_t)BUFFER_SIZE + 1];
	ssize_t		bytes_read;
	char		*line;

	if (fd < 0 || BUFFER_SIZE < 0)
		return (NULL);
	if (ft_strchr(buff, 10) >= 0)
		return (check_nl(buff));
	line = check_nl(buff);
	bytes_read = read(fd, buff, BUFFER_SIZE);
	while (bytes_read > 0)
	{
		buff[bytes_read] = '\0';
		line = ft_strjoin(line, check_nl(buff));
		if (ft_strchr(line, 10) >= 0 || !line)
			break ;
		bytes_read = read(fd, buff, BUFFER_SIZE);
	}
	if (bytes_read < 0)
